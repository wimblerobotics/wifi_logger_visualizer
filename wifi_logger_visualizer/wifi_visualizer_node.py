        ...
        for i in range(0, self.costmap_width, 20):
            for j in range(0, self.costmap_height, 20):
                ...
                if 0 <= i < self.costmap_width and 0 <= j < self.costmap_height:
                    ...
                    # print(f'wifi_logger_visualizer_node: inserted data at x={real_x}, y={real_y}, i={i}, j={j}, distance={distance}, signal_strength={signal_strength}')
                    inserted_rows += 1
            if (i % 100) == 0:
                print(f'wifi_logger_visualizer_node: processed {inserted_rows} rows of data')
