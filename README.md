...
A ROS2 package for logging WiFi signal data and visualizing it as cost maps.
...
3. **Heat Mapper Node**: Provides alternative visualization options including interactive heatmaps and value markers.
...
- `db_path`: Path to the SQLite database file (default: `wifi_data.db` in current directory)
- `scale_factor`: Scale factor for the heatmap visualization (default: 1.0)
...
1. **Standalone Mode**: Displays an interactive matplotlib heatmap with signal strength values
